...
$ProductName = "Iconify Search for PowerPoint"
...
function Find-PowerPoint {
  ...
  $known = @((Join-Path $env:ProgramFiles "Microsoft Office\root\Office16\POWERPNT.EXE"))
  if ($ProgramFilesX86) { $known += Join-Path $ProgramFilesX86 "Microsoft Office\root\Office16\POWERPNT.EXE" }
...
$powerPoint = Find-PowerPoint
if (-not $powerPoint) {
  throw "Microsoft PowerPoint was not found. Install Microsoft 365 or PowerPoint, then run this installer again."
}
...
if (Get-Process POWERPNT -ErrorAction SilentlyContinue) {
  ...
  Read-Host | Out-Null
  if (Get-Process POWERPNT -ErrorAction SilentlyContinue) {
    throw "PowerPoint is still open. Close it and run the installer again."
  }
}
...
Write-Host ""
Write-Host " 1. PowerPoint will open."
...
if (-not $NoLaunch) {
  Start-Process -FilePath $powerPoint
}
