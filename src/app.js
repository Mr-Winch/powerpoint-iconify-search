...
function bindEvents() {
  ...
  elements.refreshThemeButton.addEventListener("click", () => {
    applyOfficeTheme();
    showToast("PowerPoint theme refreshed.");
  });
...
async function insertSelected() {
  ...
  try {
    ...
    const result = await insertIconsIntoPowerPoint(items, {
      format: state.format,
    ...
    else if (result.fallbackToPng) showToast("Inserted as PNG because this PowerPoint version does not support SVG insertion.");
...
async function initialize() {
  ...
  elements.hostStatus.textContent = host ? "Connected to Microsoft PowerPoint" : "Browser preview · insertion is simulated";
